        ...
        @foreach($users as $user)
            <tr>
                ...
                <td>
                    ...
                    <form class="{{ $user->blocked ? 'unblock' : 'block' }}" 
                        action="{{ route('admin.users.toggle-block', $user) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <button title="{{ $user->blocked ? 'Unblock' : 'Block' }} User" type="submit" 
                                class="btn l-parpl btn-icon btn-icon-mini btn-round">
