...
"use strict";
window.captureVideoFrame = __webpack_require__(3);

window.swal = __webpack_require__(4);

window.toastr = __webpack_require__(36);

window.autosize = __webpack_require__(40);
...
var reInitializeSelect = function reInitializeSelect(selector) {
    return $(selector).selectpicker('refresh');
};

var refreshAllSelect = function refreshAllSelect(_) {
    return reInitializeSelect('select');
};
...
var initDataTable = function initDataTable(table) {
    var _$$DataTable;

    var addOptions = {};

    var displayAddBtn = addOptions.show ? $('.actions li a[data-action=0]').parents('li').show() : $('.actions li a[data-action=0]').parents('li').hide();

    var targetAddBtn = addOptions.target ? addOptions.target : '';

    var table = $(table).DataTable((_$$DataTable = {
        dom: 'Blfrtip'
    }, _defineProperty(_$$DataTable, 'dom', "<'row'<'col-sm-12'B>>" + "<'row'<'col-md-6 col-sm-12'l><'col-md-6 col-sm-12'f>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-md-5 col-sm-12'i><'col-md-7 col-sm-12'p>>"), _defineProperty(_$$DataTable, 'colReorder', true), _defineProperty(_$$DataTable, 'fixedHeader', true), _defineProperty(_$$DataTable, 'searchHighlight', true), _defineProperty(_$$DataTable, 'responsive', true), _defineProperty(_$$DataTable, "bStateSave", true), _defineProperty(_$$DataTable, 'language', {
    ...
    table.on('buttons-action', function (e, buttonApi, dataTable, node, config) {
        ...
        if (config._collection) {
            ...
            var menu = config._collection;

            var targetTableId = $(this).attr('id');

            var el = $('#' + targetTableId).parents('.portlet').find('.actions');

            var height = el.outerHeight(true) ? el.outerHeight(true) : 20;

            var top = el.offset().top ? el.offset().top + height : 200 + height;
